...
import 'completed_order_details_landscape_panel.dart';

class CompletedOrdersLandscapePage extends ConsumerWidget {
  const CompletedOrdersLandscapePage({super.key});
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text(
                  'Finalize checkout and payments for served orders',
                  style: TextStyle(
                    fontSize: 14,
          ...
          Expanded(
            child: Row(
              children: [
                ...
                Expanded(
                  flex: 2,
                  child: _buildOrderListSection(context, ref, state.completedOrders),
                ),
                const VerticalDivider(width: 1, thickness: 1, color: AppColors.ashGrey),
                // Right Section: Order Details
                const Expanded(
                  flex: 3,
                  child: CompletedOrderDetailsLandscapePanel(),
                ),
              ],
            ),
          ),
  ...
  Widget _buildOrderListSection(BuildContext context, WidgetRef ref, List<OrderDetailModel> orders) {
    ...
    final selectedOrderId = ref.watch(selectedCompletedOrderIdProvider);
    ...
    if (selectedOrderId != null) {
      ...
      if (!stillExists) {
        WidgetsBinding.instance.addPostFrameCallback((_) {
          ref.read(selectedCompletedOrderIdProvider.notifier).select(null);
        });
      }
    }
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Material(
                      ...
                      child: ListTile(
                        ...
                        onTap: () {
                          ref.read(selectedCompletedOrderIdProvider.notifier).select(order.orderID);
                        },
                      ),
                    );
                  },
  ...
  Widget _buildEmptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.assignment_turned_in_outlined, size: 48, color: AppColors.ashGrey.withValues(alpha: 0.5)),
          const SizedBox(height: 16),
          const Text(
            'No completed orders ready for checkout',
