...
class ConfirmedOrderDetailsLandscapePanel extends ConsumerWidget {
  const ConfirmedOrderDetailsLandscapePanel({super.key});
  ...
  Widget _buildActionButtons(BuildContext context, WidgetRef ref, OrderDetailModel order) {
    // Get all items to check if they need to be served
    final detailsAsync = ref.watch(selectedOrderDetailsProvider);
    final List<OrderDetailModel> items = detailsAsync.maybeWhen(data: (d) => d ?? [], orElse: () => []);
          ...
          children: [
            ...
            Expanded(
              child: ElevatedButton.icon(
                ...
                label: const Text('COMPLETE ORDER', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold)),
                style: ElevatedButton.styleFrom(
